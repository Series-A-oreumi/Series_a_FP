...
document.addEventListener("DOMContentLoaded", function () {
    ...
    profileChangeLink.addEventListener('click', function (e) {
        ...
        if (user_id) {
            ...
            const profileChangeUrl = `Series_a_FP/frontend/html/profile_write.html?id=${user_id}`;
            window.location.href = profileChangeUrl;
        } else {
            // 사용자가 로그인되지 않았을 때의 처리
            alert('로그인이 필요합니다.');
            window.location.href = "Series_a_FP/frontend/html/login.html";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    profileChangeLink.addEventListener('click', function (e) {
        ...
        if (user_id) {
            // user_id를 사용하여 URL을 생성하고 프로필 변경 페이지로 이동
            const profileChangeUrl = `Series_a_FP/frontend/html/profile.html?id=${user_id}`;
            window.location.href = profileChangeUrl;
        } else {
            // 사용자가 로그인되지 않았을 때의 처리
            alert('로그인이 필요합니다.');
