    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            check_alarm = Alarm.objects.filter(
                receiver=user, is_check=True).values()  # 로그인 요청을 한 유저의 이전 알람 기록들
            uncheck_alarm = Alarm.objects.filter(
                receiver=user, is_check=False).values()  # 로그인 요청을 한 유저의 확인안한 알람 기록들
    ...
    def get(self, request, user_id):
        try:
            ...
            if user.id == user_id:
                ...
                user_stories = PostSerializer(
                    stories, many=True)  # 유저가 썼던 스토리 직렬화
                ...
                studies = Study.objects.filter(author__id=user_id, public_private='public').order_by(
                    '-created_at')  # 공개되어있는 스터디, 프로젝트 게시물 보여지도록
                total_count = stories.count() + studies.count()  # 지금까지 썼던 게시글 총 개수
                ...
                user_stories = PostSerializer(
                    stories, many=True)  # 유저가 썼던 스토리 직렬화
                user_studies = StudySerializer(
                    studies, many=True)  # 유저가 썼던 스터디 직렬화
...
class RegistrationRequestList(APIView):
    ...
    def get(self, request):
        request_users = UserProfile.objects.filter(
