...
class LoginView(GenericAPIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            check_alarm = Alarm.objects.filter(
                receiver=user, is_check=True
            ).values()  # 로그인 요청을 한 유저의 이전 알람 기록들
            uncheck_alarm = Alarm.objects.filter(
                receiver=user, is_check=False
            ).values()  # 로그인 요청을 한 유저의 확인안한 알람 기록들
        ...
        try:
            ...
            if user.id == user_id:
                ...
                stories = Post.objects.filter(author=user).order_by(
                    "-created_at"
                )  # 유저가 썼던 스토리 글들 (공개, 나만보기, 기수공개 상관없이 전부)
                studies = Study.objects.filter(author=user).order_by(
                    "-created_at"
                )  # 자신이 게시했던 스터디. 프로젝트 글 목록
                ...
                stories = Post.objects.filter(author=user).order_by(
                    "-created_at"
                )  #  공개되어있는 게시물만 보여지도록 (아직 스토리 공개/비공개/기수보기 설정 안함 추후 수정 예정!)
                studies = Study.objects.filter(author=user, public_private="public").order_by(
                    "-created_at"
                )  # 공개되어있는 스터디, 프로젝트 게시물 보여지도록
                total_count = stories.count() + studies.count()  # 지금까지 썼던 게시글 총 개수
