...
from alrarm.models import Notification
...
class StudyDetail(APIView):
    ...
    def get(self, request, study_id):
        ...
        data = {"request_user": user_serializer.data, "study": post_serializer.data}
    ...
    def put(self, request, study_id):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return Response(
                    {"message": "스터디 스택을 업데이트하는 중 오류가 발생했습니다."},
...
class CommentCreate(APIView):
    ...
    def post(self, request, study_id):
        # study의 ID를 가져오는 코드 (예시로 study_id로 가정)

        # 웹소켓 메시지 생성
        message = {"type": "send.notification", "study_id": study_id, "message": "새로운 댓글이 작성되었습니다!"}

        # 웹소켓 채널 레이어를 가져오고 메시지 전송
        channel_layer = get_channel_layer()
        async_to_sync(channel_layer.group_send)(
            f"study_{study_id}", {"type": "notify", "message": "새로운 댓글이 작성되었습니다!"}
        )
        ...
        if serializer.is_valid():
            ...
            # 알림 생성 및 저장
            notification_content = f"새로운 댓글이 작성되었습니다: {comment_content}"
            comment_notification = Notification.objects.create(
                study=study, content=notification_content, User=user  # 해당 Study의 ID를 저장
            )

            messages = {"success": "댓글이 작성되었습니다."}
...
class CommentUpdateDelete(APIView):
    ...
    def delete(self, request, comment_id):
        ...
        comment.delete()
        messages = {"success": "댓글이 정상적으로 삭제되었습니다."}
    ...
    def post(self, request, study_id):
        ...
        try:
            ...
            if like.liked:
                ...
                messages = {"cancel": f"{study.author} 게시물 좋아요를 취소했습니다."}
                return Response(messages, status=status.HTTP_204_NO_CONTENT)
            # 좋아요를 누르지 않았던 경우, 좋아요를 추가합니다.
                ...
                messages = {"success": f"{study.author} 게시물 좋아요를 눌렀습니다."}
                return Response(messages, status=status.HTTP_201_CREATED)
